...
break at any time. Pin the `minor version <https://semver.org/>`_ with a wildcard (ex. ``==4.0.*``,
not ``==4.0.8``)—or reinstall with the ``--upgrade`` (as shown below) often—to ensure you always get the latest
stable release.
